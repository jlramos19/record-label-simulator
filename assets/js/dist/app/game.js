...
function setSlotValue(targetId, value) {
    ...
    if (targetId === "track-writer")
        state.ui.trackSlots.songwriterId = value;
    if (targetId === "track-performer")
        state.ui.trackSlots.performerId = value;
    if (targetId === "track-producer")
        state.ui.trackSlots.producerId = value;
    if (targetId === "era-act")
...
function renderSlots() {
    ...
    if ($("trackWriterSlot"))
        $("trackWriterSlot").textContent = trackWriter ? trackWriter.name : "Unassigned";
    if ($("trackPerformerSlot"))
        $("trackPerformerSlot").textContent = trackPerformer ? trackPerformer.name : "Unassigned";
    if ($("trackProducerSlot"))
        $("trackProducerSlot").textContent = trackProducer ? trackProducer.name : "Unassigned";
    if ($("eraActSlot"))
...
function computeQualityPotential(track) {
    ...
    if (!writer || !performer || !producer)
        return clampQuality(60);
    let score = 40 + writer.skill * 0.2 + performer.skill * 0.2 + producer.skill * 0.3 + rand(-5, 5);
    const prefHits = (writer.prefThemes.includes(track.theme) ? 1 : 0) +
...
function scheduleStage(track, stageIndex) {
    ...
    const creatorId = stage.role === "Songwriter"
...
function tryStartWorkOrder(order) {
    ...
    if (order.status !== "Queued")
        return;
    if (state.studio.inUse >= state.studio.slots)
        return;
    const creator = getCreator(order.creatorId);
    if (!creator || creator.stamina < stage.stamina)
        return;
...
function normalizeState() {
    ...
    if (!state.ui.trackSlots)
        state.ui.trackSlots = { actId: null, songwriterId: null, performerId: null, producerId: null };
    ...
    if (!state.economy)
        state.economy = { lastRevenue: 0, lastUpkeep: 0, lastWeek: 0 };
    if (!state.era)
...
export { session, state, $, clamp, formatMoney, formatCount, formatDate, openOverlay, closeOverlay, logEvent, makeTrackTitle, makeProjectTitle, makeLabelName, makeActName, makeEraName, handleFromName, makeAct, createTrack, getModifier, getAct, getCreator, getTrack, getStudioAvailableSlots, getEraById, getActiveEras, getFocusedEra, setFocusEraById, startEraForAct, endEraById, pickDistinct, uid, weekIndex, normalizeCreator, postCreatorSigned, renderAll, renderStats, renderSlots, renderActs, renderCreators, renderTracks, renderReleaseDesk, renderEraStatus, renderWallet, renderActiveCampaigns, renderQuickRecipes, renderCalendarList, renderGenreIndex, renderCharts, renderSocialFeed, renderMainMenu, updateGenrePreview, formatWeekRangeLabel, renderAutoAssignModal, rankCandidates, recommendTrackPlan, recommendReleasePlan, recommendProjectType, assignToSlot, clearSlot, getSlotElement, describeSlot, setSlotTarget, updateActMemberFields, advanceHours, releaseTrack, scheduleRelease, acceptBailout, declineBailout, refreshSelectOptions, computeCharts, buildMarketCreators, startGameLoop, setTimeSpeed, openMainMenu, closeMainMenu, saveToActiveSlot, getSlotData, loadSlot, resetState, deleteSlot };
