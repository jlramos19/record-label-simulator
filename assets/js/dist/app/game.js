...
import { alignmentClass, countryColor, countryDemonym, formatGenreKeyLabel, formatGenreLabel, makeGenre, moodFromGenre, resolveLabelAcronym, slugify, themeFromGenre } from "./game/label-utils.js";
...
function makeDefaultState() {
    ...
    return {
        ...
        distribution: {
            inventoryByRegion: {},
            pendingUsageWeek: null,
            pendingUsageByRegion: {},
            lastAppliedWeek: null,
            lastUsageByRegion: {},
            lastWarningWeek: null
        },
        scheduledEvents: [],
        marketTracks: [],
        trends: [],
...
function namePartKey(part) {
    ...
    if (typeof part === "string")
        return part.trim();
    if (typeof part === "object" && part.romanized)
        return String(part.romanized).trim();
...
function resolveGenderedNamePool(genderKey, pools, fallback = []) {
    ...
    if (genderKey === "nonbinary")
        merged = [...man, ...woman, ...nonbinary];
    if (genderKey === "man")
        merged = [...man, ...nonbinary];
    if (genderKey === "woman")
        merged = [...woman, ...nonbinary];
...
function resolveNationAgeProfile(nation) {
    if (typeof NATION_AGE_PYRAMID_PROFILES !== "undefined" && NATION_AGE_PYRAMID_PROFILES) {
        ...
        if (profile)
            return normalizeAgeProfile(profile);
    }
...
function buildAudienceAgeGroupWeights(profile = DEFAULT_AGE_PYRAMID_PROFILE) {
    ...
    return AUDIENCE_AGE_GROUPS.map((group) => {
        ...
        if (midpoint < 20)
            weight += safe.childBoost;
        if (midpoint >= 60)
            weight *= safe.seniorDrop;
...
function mergeAgeGroupDistributions(total, nationEntries) {
    ...
    nationEntries.forEach((entry) => {
        if (!Array.isArray(entry?.ageGroups))
            return;
...
function resolveAudienceChunkTarget(totalPopulation) {
    if (!Number.isFinite(totalPopulation))
        return AUDIENCE_CHUNK_TARGET_DEFAULT;
    if (totalPopulation < 2000000000)
        return AUDIENCE_CHUNK_TARGET_MIN;
    if (totalPopulation < 6000000000)
        return clamp(60, AUDIENCE_CHUNK_TARGET_MIN, AUDIENCE_CHUNK_TARGET_MAX);
    if (totalPopulation < 9000000000)
        return clamp(75, AUDIENCE_CHUNK_TARGET_MIN, AUDIENCE_CHUNK_TARGET_MAX);
...
function buildAudienceLifeExpectancy(age) {
    ...
    if (minAge < age + 4)
        minAge = Math.min(116, Math.floor((age + 4) / 4) * 4);
...
const DEFAULT_OCCUPATION_SHARES = { factory: 0.02, shopping: 0.025, broadcast: 0.01 };
function resolveOccupationShares() {
    const base = typeof AUDIENCE_OCCUPATION_SHARES === "object" && AUDIENCE_OCCUPATION_SHARES
        ? AUDIENCE_OCCUPATION_SHARES
        : DEFAULT_OCCUPATION_SHARES;
    return {
        factory: clamp(Number(base.factory) || 0, 0, 1),
        shopping: clamp(Number(base.shopping) || 0, 0, 1),
        broadcast: clamp(Number(base.broadcast) || 0, 0, 1)
    };
}
function resolveAudienceRegionId(nation, rng = Math.random) {
    ...
    const share = clamp(Number(REGION_CAPITAL_SHARE) || 0.35, 0, 1);
    return roll < share ? capital.id : elsewhere.id;
}
function buildAudienceOccupations({ age, size } = {}) {
    const shares = resolveOccupationShares();
    const minAge = Number.isFinite(AUDIENCE_OCCUPATION_MIN_AGE) ? AUDIENCE_OCCUPATION_MIN_AGE : 16;
    const maxAge = Number.isFinite(AUDIENCE_OCCUPATION_MAX_AGE) ? AUDIENCE_OCCUPATION_MAX_AGE : 70;
    const population = Math.max(0, Math.round(Number(size) || 0));
    const workingEligible = Number.isFinite(age) ? age >= minAge && age <= maxAge : true;
    const workforce = workingEligible ? population : 0;
    const factory = Math.round(workforce * shares.factory);
    const shopping = Math.round(workforce * shares.shopping);
    const broadcast = Math.round(workforce * shares.broadcast);
    const used = factory + shopping + broadcast;
    const consumer = Math.max(0, population - used);
    return { factory, shopping, broadcast, consumer };
}
function makeAudienceChunk({ nation, year, age, parent = null, snapshot = null } = {}) {
    ...
    if (parent)
        driftAudiencePreferences(preferences);
    const profile = pickAudienceTimeProfile(resolvedAge);
    const regionId = resolveAudienceRegionId(resolvedNation);
    const occupations = buildAudienceOccupations({ age: resolvedAge, size: AUDIENCE_CHUNK_SIZE });
    return {
        id: uid("AU"),
...
function normalizeAudienceChunk(chunk) {
    ...
    const region = REGION_DEFS.find((entry) => entry.id === normalized.regionId);
    if (!normalized.regionId || !region || region.nation !== normalized.nation) {
        normalized.regionId = resolveAudienceRegionId(normalized.nation);
    }
    ...
    if (!Number.isFinite(normalized.weeklyBudget))
        normalized.weeklyBudget = computeAudienceWeeklyBudget(normalized.age);
    if (!Number.isFinite(normalized.engagementRate))
        normalized.engagementRate = computeAudienceEngagementRate(normalized.age);
    if (!Number.isFinite(normalized.weeklyHours))
        normalized.weeklyHours = computeAudienceWeeklyHours(normalized.age);
    if (!Number.isFinite(normalized.reproductionRate))
        normalized.reproductionRate = computeAudienceReproductionRate(normalized.age);
    if (!Number.isFinite(normalized.emigrationRate))
        normalized.emigrationRate = computeAudienceEmigrationRate(normalized.age);
    ...
    if (!Number.isFinite(normalized.size))
        normalized.size = AUDIENCE_CHUNK_SIZE;
    normalized.occupations = buildAudienceOccupations({ age: normalized.age, size: normalized.size });
    if (!Number.isFinite(normalized.createdYear))
        normalized.createdYear = currentYear();
    return normalized;
}
function ensureAudienceChunksStore() {
    ...
    if (!Array.isArray(state.audienceChunks.chunks))
        state.audienceChunks.chunks = [];
    if (!Number.isFinite(state.audienceChunks.lastUpdateYear))
        state.audienceChunks.lastUpdateYear = null;
    if (!Number.isFinite(state.audienceChunks.lastUpdateAt))
        state.audienceChunks.lastUpdateAt = null;
    return state.audienceChunks;
}
function ensureDistributionStore() {
    if (!state.distribution || typeof state.distribution !== "object") {
        state.distribution = {
            inventoryByRegion: {},
            pendingUsageWeek: null,
            pendingUsageByRegion: {},
    ...
    if (!state.distribution.inventoryByRegion || typeof state.distribution.inventoryByRegion !== "object") {
        state.distribution.inventoryByRegion = {};
    }
    if (!state.distribution.pendingUsageByRegion || typeof state.distribution.pendingUsageByRegion !== "object") {
        state.distribution.pendingUsageByRegion = {};
    }
    if (!state.distribution.lastUsageByRegion || typeof state.distribution.lastUsageByRegion !== "object") {
        state.distribution.lastUsageByRegion = {};
    }
    if (typeof state.distribution.pendingUsageWeek !== "number")
        state.distribution.pendingUsageWeek = null;
    if (typeof state.distribution.lastAppliedWeek !== "number")
        state.distribution.lastAppliedWeek = null;
    if (typeof state.distribution.lastWarningWeek !== "number")
        state.distribution.lastWarningWeek = null;
...
function advanceAudienceChunksYear(year = currentYear()) {
    ...
    store.chunks.forEach((chunk) => {
        ...
        if (Math.random() < (chunk.emigrationRate || 0)) {
            ...
            if (options.length) {
                chunk.nation = pickOne(options);
                chunk.regionId = resolveAudienceRegionId(chunk.nation);
            }
        }
...
const DEFAULT_DISTRIBUTION_CAPACITY = { factoryUnitsPerStaffWeek: 25, storeUnitsPerStaffWeek: 20 };
function resolveDistributionCapacityConfig() {
    const raw = typeof DISTRIBUTION_CAPACITY === "object" && DISTRIBUTION_CAPACITY
        ? DISTRIBUTION_CAPACITY
        : DEFAULT_DISTRIBUTION_CAPACITY;
    return {
        factoryUnitsPerStaffWeek: Math.max(0, Number(raw.factoryUnitsPerStaffWeek) || DEFAULT_DISTRIBUTION_CAPACITY.factoryUnitsPerStaffWeek),
        storeUnitsPerStaffWeek: Math.max(0, Number(raw.storeUnitsPerStaffWeek) || DEFAULT_DISTRIBUTION_CAPACITY.storeUnitsPerStaffWeek)
    };
}
function resolveRegionPopulation(snapshot, regionId) {
    if (!snapshot || !regionId)
        return 0;
    const region = REGION_DEFS.find((entry) => entry.id === regionId);
    if (!region)
        return 0;
    const nation = snapshot.nations.find((entry) => entry.nation === region.nation);
    if (!nation)
        return 0;
    const lowerId = String(region.id).toLowerCase();
    if (lowerId.includes("capital"))
        return nation.capital || 0;
    if (lowerId.includes("elsewhere"))
        return nation.elsewhere || 0;
    return nation.total || 0;
}
function buildDistributionSnapshot() {
    const store = ensureDistributionStore();
    const snapshot = computePopulationSnapshot();
    const chunks = getAudienceChunksSnapshot().chunks || [];
    const capacityConfig = resolveDistributionCapacityConfig();
    const regionSample = {};
    REGION_DEFS.forEach((region) => {
        regionSample[region.id] = {
            regionId: region.id,
    ...
    chunks.forEach((chunk) => {
        const regionId = chunk.regionId || resolveAudienceRegionId(chunk.nation);
        if (!regionId || !regionSample[regionId])
            return;
        const sample = regionSample[regionId];
        const members = Math.max(0, Math.round(Number(chunk.size) || 0));
        sample.sampleMembers += members;
        sample.sampleFactory += Math.max(0, Math.round(Number(chunk.occupations?.factory) || 0));
        sample.sampleShopping += Math.max(0, Math.round(Number(chunk.occupations?.shopping) || 0));
        sample.sampleBroadcast += Math.max(0, Math.round(Number(chunk.occupations?.broadcast) || 0));
    });
    const regions = {};
    REGION_DEFS.forEach((region) => {
        const sample = regionSample[region.id] || {
            sampleMembers: 0,
            sampleFactory: 0,
            sampleShopping: 0,
            sampleBroadcast: 0
        };
        const population = resolveRegionPopulation(snapshot, region.id);
        const scale = sample.sampleMembers > 0 ? population / sample.sampleMembers : 0;
        const factoryStaff = Math.round(sample.sampleFactory * scale);
        const shoppingStaff = Math.round(sample.sampleShopping * scale);
        const broadcastStaff = Math.round(sample.sampleBroadcast * scale);
        const production = roundToAudienceChunk(factoryStaff * capacityConfig.factoryUnitsPerStaffWeek);
        const storeCapacity = roundToAudienceChunk(shoppingStaff * capacityConfig.storeUnitsPerStaffWeek);
        const inventory = Math.max(0, Math.round(Number(store.inventoryByRegion?.[region.id] || 0)));
        const lastUsage = store.lastUsageByRegion?.[region.id] || {};
        regions[region.id] = {
            regionId: region.id,
            regionLabel: region.label || region.id,
            nation: region.nation,
            population,
            sampleMembers: sample.sampleMembers,
            staff: {
                factory: factoryStaff,
                shopping: shoppingStaff,
            ...
            inventory,
            lastDemand: Math.max(0, Math.round(Number(lastUsage.demand || 0))),
            lastSold: Math.max(0, Math.round(Number(lastUsage.sold || 0)))
        };
    });
    const nations = {};
    NATIONS.forEach((nation) => {
        const regionIds = REGION_DEFS.filter((region) => region.nation === nation).map((region) => region.id);
        const aggregate = regionIds.reduce((totals, regionId) => {
            const region = regions[regionId];
            if (!region)
                return totals;
            totals.population += region.population;
            totals.inventory += region.inventory;
            totals.production += region.capacity.production;
            totals.storeCapacity += region.capacity.store;
            totals.lastDemand += region.lastDemand;
            totals.lastSold += region.lastSold;
            return totals;
        }, { population: 0, inventory: 0, production: 0, storeCapacity: 0, lastDemand: 0, lastSold: 0 });
        nations[nation] = { nation, ...aggregate };
    });
    const global = Object.values(nations).reduce((totals, nation) => {
        totals.population += nation.population;
        totals.inventory += nation.inventory;
        totals.production += nation.production;
        totals.storeCapacity += nation.storeCapacity;
        totals.lastDemand += nation.lastDemand;
        totals.lastSold += nation.lastSold;
        return totals;
    }, { population: 0, inventory: 0, production: 0, storeCapacity: 0, lastDemand: 0, lastSold: 0 });
    return { regions, nations, global };
}
function roundDownToAudienceChunk(value) {
    const chunk = Number.isFinite(AUDIENCE_CHUNK_SIZE) ? AUDIENCE_CHUNK_SIZE : 1000;
    return Math.floor(Math.max(0, value) / chunk) * chunk;
}
function applyDistributionAdjustmentsForScope(entries, capacity, scopeId) {
    const config = resolveReleaseDistributionConfig();
    const physicalShare = config.physicalShare;
    const bucket = [];
    let totalDesiredPhysical = 0;
    (entries || []).forEach((entry) => {
        if (!entry)
            return;
        const metrics = entry.metrics || {};
        const baseSales = Math.max(0, Number(metrics.sales || 0));
        const track = entry.track || null;
        if (!track || baseSales <= 0) {
            metrics.salesPhysical = 0;
            metrics.salesDigital = roundDownToAudienceChunk(baseSales);
            metrics.sales = roundToAudienceChunk(metrics.salesPhysical + metrics.salesDigital);
            entry.metrics = metrics;
            return;
        }
        const isPlayer = typeof track.isPlayer === "boolean" ? track.isPlayer : Boolean(track.actId);
        const labelFans = isPlayer ? Number(state.label?.fans || 0) : null;
        const distResult = resolveReleaseDistributionForTrack(track, track.distribution || "Digital", { labelFans });
        const dist = distResult.distribution;
        let desiredPhysical = 0;
        let desiredDigital = baseSales;
        if (dist === "Physical") {
            desiredPhysical = baseSales;
            desiredDigital = 0;
        }
        else if (dist === "Both") {
            desiredPhysical = baseSales * physicalShare;
            desiredDigital = baseSales - desiredPhysical;
        }
        bucket.push({ entry, metrics, desiredPhysical, desiredDigital });
        totalDesiredPhysical += desiredPhysical;
    });
    const inventory = Math.max(0, Number(capacity?.inventory || 0));
    const production = Math.max(0, Number(capacity?.production || 0));
    const storeCapacity = Math.max(0, Number(capacity?.storeCapacity || 0));
    const supply = inventory + production;
    const sellCap = Math.max(0, Math.min(supply, storeCapacity));
    const ratio = totalDesiredPhysical > 0 ? Math.min(1, sellCap / totalDesiredPhysical) : 0;
    let soldTotal = 0;
    bucket.forEach((item) => {
        const actualPhysical = item.desiredPhysical > 0 ? roundDownToAudienceChunk(item.desiredPhysical * ratio) : 0;
        const actualDigital = item.desiredDigital > 0 ? roundDownToAudienceChunk(item.desiredDigital) : 0;
        item.metrics.salesPhysical = actualPhysical;
        item.metrics.salesDigital = actualDigital;
        item.metrics.sales = roundToAudienceChunk(actualPhysical + actualDigital);
        item.entry.metrics = item.metrics;
        soldTotal += actualPhysical;
    });
    return {
        scopeId: scopeId || "global",
        demand: roundToAudienceChunk(totalDesiredPhysical),
        sold: soldTotal,
        sellCap,
        supply,
        storeCap: storeCapacity
    };
}
function logDistributionCapacityWarnings(usageByRegion) {
    const store = ensureDistributionStore();
    const week = weekIndex() + 1;
    if (store.lastWarningWeek === week)
        return;
    const constrained = Object.entries(usageByRegion || {})
        .filter(([, usage]) => usage && usage.demand > usage.sellCap && usage.sellCap > 0);
    if (!constrained.length)
        return;
    constrained.sort((a, b) => (b[1].demand - b[1].sellCap) - (a[1].demand - a[1].sellCap));
    const top = constrained[0];
    const topRegion = REGION_DEFS.find((region) => region.id === top[0]);
    const topLabel = topRegion?.label || top[0];
    logEvent(`Physical demand exceeded store capacity in ${constrained.length} region${constrained.length === 1 ? "" : "s"} (top: ${topLabel}).`, "warn");
    store.lastWarningWeek = week;
}
function applyDistributionAdjustments({ globalScores, nationScores, regionScores } = {}) {
    const snapshot = buildDistributionSnapshot();
    const usageByRegion = {};
    const nationUsage = {};
    REGION_DEFS.forEach((region) => {
        const entries = regionScores?.[region.id] || [];
        usageByRegion[region.id] = applyDistributionAdjustmentsForScope(entries, {
            inventory: snapshot.regions?.[region.id]?.inventory || 0,
            production: snapshot.regions?.[region.id]?.capacity?.production || 0,
            storeCapacity: snapshot.regions?.[region.id]?.capacity?.store || 0
        }, region.id);
    });
    NATIONS.forEach((nation) => {
        const entries = nationScores?.[nation] || [];
        nationUsage[nation] = applyDistributionAdjustmentsForScope(entries, {
            inventory: snapshot.nations?.[nation]?.inventory || 0,
            production: snapshot.nations?.[nation]?.production || 0,
            storeCapacity: snapshot.nations?.[nation]?.storeCapacity || 0
        }, nation);
    });
    const globalUsage = applyDistributionAdjustmentsForScope(globalScores || [], {
        inventory: snapshot.global?.inventory || 0,
        production: snapshot.global?.production || 0,
        storeCapacity: snapshot.global?.storeCapacity || 0
    }, "global");
    const store = ensureDistributionStore();
    store.pendingUsageWeek = weekIndex() + 1;
    store.pendingUsageByRegion = usageByRegion;
    logDistributionCapacityWarnings(usageByRegion);
    return { snapshot, usageByRegion, nationUsage, globalUsage };
}
function applyDistributionInventoryForWeek() {
    const store = ensureDistributionStore();
    const week = weekIndex() + 1;
    if (store.lastAppliedWeek === week)
        return false;
    const snapshot = buildDistributionSnapshot();
    const usage = store.pendingUsageByRegion || {};
    REGION_DEFS.forEach((region) => {
        const regionId = region.id;
        const capacity = snapshot.regions?.[regionId]?.capacity || { production: 0 };
        const currentInventory = Math.max(0, Math.round(Number(store.inventoryByRegion?.[regionId] || 0)));
        const sold = Math.max(0, Math.round(Number(usage[regionId]?.sold || 0)));
        const nextInventory = Math.max(0, Math.round(currentInventory + capacity.production - sold));
        store.inventoryByRegion[regionId] = nextInventory;
    });
    store.lastAppliedWeek = week;
    store.lastUsageByRegion = { ...usage };
    return true;
}
function getDistributionSnapshot() {
...
function normalizeCreatorGenderKey(genderIdentity) {
    ...
    if (!raw)
        return null;
    if (["man", "male", "m"].includes(raw))
        return "man";
    if (["woman", "female", "f"].includes(raw))
        return "woman";
    if (["nonbinary", "non-binary", "nb", "enby"].includes(raw))
        return "nonbinary";
    return null;
}
...
function pickCreatorPortraitGender(preferredGenres, ageBin, countryKey, entries) {
    if (!preferredGenres?.length || !ageBin || !countryKey || !entries)
        return null;
    const available = CREATOR_PORTRAIT_GENDER_KEYS.filter((gender) => (preferredGenres.some((genreKey) => {
...
function resolveCreatorPortraitGenre(creator) {
    ...
    if (!theme || !THEMES.includes(theme) || !mood || !MOODS.includes(mood))
        return null;
...
function buildCreatorPortraitKey(creator) {
    ...
    if (!genre || !ageBin || !countryKey || !genderKey)
        return null;
...
function setCreatorPortraitNote(creator, note) {
    ...
    if (creator.portraitNote !== next)
        creator.portraitNote = next;
}
function assignCreatorPortrait(creator) {
    ...
    if (!creator.portraitGenreKey && existing?.genreKey) {
        ...
        if (normalizedGenre)
            creator.portraitGenreKey = normalizedGenre;
    }
    ...
    if (!preferredGenres.length)
        issues.push("theme/mood prefs");
    if (!ageBin)
        issues.push("age bin");
    if (!countryKey)
        issues.push("nationality");
    if (!genderKey)
        issues.push("gender identity");
    if (issues.length) {
    ...
    if (!genreKey && creator.portraitGenreKey)
        creator.portraitGenreKey = null;
    ...
    if (!genreKey) {
        if (!availableGenreKeys.length) {
            setCreatorPortraitNote(creator, `No portraits found for ${ageBin}/${identityKey} across preferred genres.`);
            return trimmedUrl || null;
        }
...
function keepEraRolloutPlansEnabled() {
    if (typeof state.meta?.keepEraRolloutPlans === "boolean")
        return state.meta.keepEraRolloutPlans;
    if (typeof state.meta?.keepEraRolloutHusks === "boolean")
        return state.meta.keepEraRolloutHusks;
    return true;
}
function generateEraRolloutHusk(era) {
    if (!era || !keepEraRolloutPlansEnabled())
        return null;
    if (era.rolloutPlanGenerated || era.rolloutHuskGenerated)
        return null;
...
function resolveCreatorGenderIdentity({ genderIdentity = null, country = null, age = null, prefThemes = [], prefMoods = [] } = {}) {
...
function normalizeCreator(creator) {
    ...
    if (creator.portraitGenreKey && !creator.portraitGenreKey.trim())
        creator.portraitGenreKey = null;
    ...
    if (!creator.portraitGenreKey && creator.portraitUrl) {
        ...
        if (inferredKey)
            creator.portraitGenreKey = inferredKey;
    }
    ...
    if (creator.portraitNote && !creator.portraitNote.trim())
...
function updateCreatorCatharsisInactivity(creator, dayIndex, { log = true } = {}) {
    ...
    if (creator.catharsisInactivityDay === resolvedDay)
        return;
    ...
    if (inactiveDays <= CREATOR_CATHARSIS_INACTIVITY_GRACE_DAYS) {
        ...
        if (startDay !== null && prevPct > 0) {
            ...
            if (daysSince >= CREATOR_CATHARSIS_INACTIVITY_RECOVERY_DAYS) {
                ...
                const basePct = clampCatharsisInactivityPct(Number.isFinite(creator.catharsisInactivityRecoveryStartPct)
    ...
    if (!log || prevPct === nextPct)
        return;
    if (prevPct === 0 && nextPct > 0) {
        logEvent(`Inactivity debuff: ${creator.name} [${creator.id}] now has a ${Math.round(nextPct * 100)}% catharsis penalty.`, "warn");
        return;
    }
    if (prevPct > 0 && nextPct === 0) {
    ...
    if (prevPct < CREATOR_CATHARSIS_INACTIVITY_MAX_PCT && nextPct === CREATOR_CATHARSIS_INACTIVITY_MAX_PCT) {
        logEvent(`Inactivity debuff capped: ${creator.name} [${creator.id}] reached ${Math.round(nextPct * 100)}% catharsis penalty.`, "warn");
    }
}
...
function applyActStaminaSpend(creatorIds, totalCost, { roster = null, context = {}, activityLabel = "Act activity", logOverdraw = true, skillGainPerStamina = 0 } = {}) {
    ...
    if (logOverdraw && pooled.overdraw > 0) {
        ...
        logEvent(`Act stamina overdraw${label}: spent ${formatCount(pooled.totalCost)} but crew had ${formatCount(pooled.totalAvailable)}.`, "warn");
    }
...
const STATE_INDEXES = {
    ...
    marketTrackArchive: { label: "market-track-archive", source: null, length: 0, map: new Map(), key: (entry) => entry?.id || null },
    marketTrackArchiveByTrackId: { label: "market-track-archive-by-track", source: null, length: 0, map: new Map(), key: (entry) => entry?.trackId || null },
...
function bumpIdCounterFromId(value) {
    ...
    if (Number.isFinite(num) && num > idCounter)
        idCounter = num;
}
function injectCheaterMarketCreators({ ids = [], role = "Songwriter", age = null, country = null, genderIdentity = null, theme = null, mood = null, portraitKey = null, portraitFile = null } = {}) {
    if (!state.meta?.cheaterMode) {
    ...
    idList.forEach((rawId) => {
        ...
        if (resolvedCountry)
            creator.country = resolvedCountry;
        if (Number.isFinite(resolvedAge))
            creator.age = resolvedAge;
        if (resolvedTheme)
            creator.prefThemes = fillDistinct([resolvedTheme], THEMES, 2);
        if (resolvedMood)
            creator.prefMoods = fillDistinct([resolvedMood], MOODS, 2);
        if (resolvedGender)
            creator.genderIdentity = resolvedGender;
        if (portraitUrl) {
    ...
    if (portraitKeyWarning)
        logEvent(portraitKeyWarning, "warn");
    if (portraitFileWarning)
        logEvent(portraitFileWarning, "warn");
    if (created.length) {
...
function resolveUniqueProjectName(baseName) {
    ...
    for (let i = 2; i <= 24; i += 1) {
        ...
        if (!existing.has(normalizeProjectName(candidate)))
            return candidate;
    }
    return `${baseName} II`;
}
...
function getArchivedMarketTrackById(id) {
    return getStateEntry("marketTrackArchive", state.meta?.marketTrackArchive, id);
}
function getArchivedMarketTrackByTrackId(trackId) {
    return getStateEntry("marketTrackArchiveByTrackId", state.meta?.marketTrackArchive, trackId);
}
...
function removeTrackFromEraContent(trackId) {
    ...
    let removed = 0;
    eras.forEach((era) => {
        if (!era || !Array.isArray(era.contentItems))
            return;
...
function clearTrackUiSelections(trackId) {
    if (!state.ui)
        return;
    if (state.ui.createTrackId === trackId)
        state.ui.createTrackId = null;
    if (state.ui.createTrackIds) {
        if (state.ui.createTrackIds.demo === trackId)
            state.ui.createTrackIds.demo = null;
        if (state.ui.createTrackIds.master === trackId)
            state.ui.createTrackIds.master = null;
    }
    if (state.ui.promoSlots?.trackId === trackId)
    ...
    if (state.ui.socialSlots?.trackId === trackId)
        state.ui.socialSlots.trackId = null;
    if (state.ui.awardBidTrackId === trackId)
...
function scrapTrack(trackId, { reason } = {}) {
    ...
    const details = [];
    if (removedOrders)
        details.push(`${removedOrders} work order${removedOrders === 1 ? "" : "s"}`);
    if (removedQueue)
        details.push(`${removedQueue} release queue entr${removedQueue === 1 ? "y" : "ies"}`);
    if (rolloutRemoved.drops)
        details.push(`${rolloutRemoved.drops} rollout drop${rolloutRemoved.drops === 1 ? "" : "s"}`);
    if (rolloutRemoved.events)
        details.push(`${rolloutRemoved.events} rollout event${rolloutRemoved.events === 1 ? "" : "s"}`);
    if (removedScheduled)
        details.push(`${removedScheduled} scheduled event${removedScheduled === 1 ? "" : "s"}`);
    if (eraRemoved)
        details.push(`${eraRemoved} era content tag${eraRemoved === 1 ? "" : "s"}`);
...
function rolloutEventLabel(actionType) {
    if (actionType === "awardShow")
        return "Award Show";
    if (actionType === "awardPerformance")
...
const DEFAULT_RELEASE_DISTRIBUTION_CONFIG = {
    digitalLeadWeeks: 1,
    physicalLeadWeeks: 2,
    digitalRushFee: 1000,
...
function resolveReleaseDistributionConfig() {
    const raw = typeof RELEASE_DISTRIBUTION === "object" && RELEASE_DISTRIBUTION
        ? RELEASE_DISTRIBUTION
        : DEFAULT_RELEASE_DISTRIBUTION_CONFIG;
    return {
        digitalLeadWeeks: Math.max(0, Number(raw.digitalLeadWeeks) || DEFAULT_RELEASE_DISTRIBUTION_CONFIG.digitalLeadWeeks),
        physicalLeadWeeks: Math.max(0, Number(raw.physicalLeadWeeks) || DEFAULT_RELEASE_DISTRIBUTION_CONFIG.physicalLeadWeeks),
        digitalRushFee: Math.max(0, Number(raw.digitalRushFee) || DEFAULT_RELEASE_DISTRIBUTION_CONFIG.digitalRushFee),
        physicalMinQuality: Math.max(0, Number(raw.physicalMinQuality) || DEFAULT_RELEASE_DISTRIBUTION_CONFIG.physicalMinQuality),
        physicalMinFans: Math.max(0, Number(raw.physicalMinFans) || DEFAULT_RELEASE_DISTRIBUTION_CONFIG.physicalMinFans),
        physicalShare: clamp(Number(raw.physicalShare) || DEFAULT_RELEASE_DISTRIBUTION_CONFIG.physicalShare, 0, 1)
    };
}
function resolveDistributionLeadWeeks(distribution, { rush = false } = {}) {
    const config = resolveReleaseDistributionConfig();
    const dist = distribution || "Digital";
    if ((dist === "Physical" || dist === "Both"))
        return config.physicalLeadWeeks;
    return rush ? 0 : config.digitalLeadWeeks;
}
function getReleaseRushFee(distribution, { rush = false } = {}) {
    if (!rush)
        return 0;
    if (distribution !== "Digital")
        return 0;
    const config = resolveReleaseDistributionConfig();
    return Math.max(0, config.digitalRushFee || 0);
}
function applyReleaseRushFee(distribution, { rush = false } = {}) {
    const fee = getReleaseRushFee(distribution, { rush });
    if (!fee)
...
function getReleaseAsapAtForDistribution(distribution, { rush = false } = {}) {
    const leadWeeks = resolveDistributionLeadWeeks(distribution, { rush });
    const leadHours = leadWeeks * WEEK_HOURS;
    return alignReleaseAtToProcessing(state.time.epochMs + leadHours * HOUR_MS);
}
function resolveReleaseScheduleWindow(requestedAt, { distribution, rush = false } = {}) {
    const now = state.time.epochMs;
    const leadWeeks = resolveDistributionLeadWeeks(distribution, { rush });
    const leadHours = leadWeeks * WEEK_HOURS;
    const earliestAt = alignReleaseAtToProcessing(now + leadHours * HOUR_MS);
    const alignedRequested = alignReleaseAtToProcessing(requestedAt);
    const releaseAt = alignedRequested < earliestAt ? earliestAt : alignedRequested;
    return { releaseAt, alignedRequested, earliestAt, delayed: alignedRequested < earliestAt, leadWeeks };
}
function evaluatePhysicalEligibility(track, { labelFans } = {}) {
    const config = resolveReleaseDistributionConfig();
    const quality = resolveTrackQualityScore(track);
    const minQuality = Number.isFinite(config.physicalMinQuality) ? config.physicalMinQuality : 0;
    const minFans = Number.isFinite(config.physicalMinFans) ? config.physicalMinFans : 0;
    const fans = Number.isFinite(labelFans) ? labelFans : null;
    const qualityOk = !Number.isFinite(minQuality) || (Number.isFinite(quality) && quality >= minQuality);
    const fansOk = !Number.isFinite(minFans) || !Number.isFinite(fans) || fans >= minFans;
    return {
        ok: qualityOk && fansOk,
...
function resolveReleaseDistributionForTrack(track, distribution, { labelFans, log } = {}) {
    const dist = distribution || track?.distribution || "Digital";
    if (dist !== "Physical" && dist !== "Both")
        return { distribution: dist, downgraded: false, eligibility: null };
    const eligibility = evaluatePhysicalEligibility(track, { labelFans });
    if (eligibility.ok)
        return { distribution: dist, downgraded: false, eligibility };
    if (log) {
        const reasons = [];
        if (!eligibility.qualityOk)
            reasons.push(`quality ${eligibility.quality ?? "?"} < ${eligibility.minQuality}`);
        if (!eligibility.fansOk)
            reasons.push(`fans ${formatCount(eligibility.fans ?? 0)} < ${formatCount(eligibility.minFans)}`);
        logEvent(`Physical distribution blocked (${reasons.join(", ") || "eligibility fail"}). Switching to Digital.`, "warn");
    }
    return { distribution: "Digital", downgraded: true, eligibility };
}
...
function applyReleaseFees({ distribution, rush = false, track = null } = {}) {
    const distFee = getReleaseDistributionFee(distribution);
    const rushFee = getReleaseRushFee(distribution, { rush });
    const total = Math.max(0, distFee + rushFee);
    if (total && state.label.cash < total) {
        return { ok: false, total, distributionFee: distFee, rushFee };
    }
    if (total)
        state.label.cash -= total;
    if (track && distFee)
        recordTrackDistributionFee(track, distFee);
    if (track && rushFee)
        recordTrackDistributionFee(track, rushFee);
    return { ok: true, total, distributionFee: distFee, rushFee };
}
...
function releaseTrack(track, note, distribution, { chargeFee = false } = {}) {
    ...
    const distResult = resolveReleaseDistributionForTrack(track, distribution || track.distribution || "Digital", {
...
function scheduleRelease(track, hoursFromNow, distribution, note, { rush = false } = {}) {
    if (!track || !track.actId || !getAct(track.actId)) {
    ...
    if (scheduleHours === 0 && !rush) {
        logEvent("Scheduled releases must target a future time. Defaulting to +1h.", "warn");
        scheduleHours = 1;
    }
    const distResult = resolveReleaseDistributionForTrack(track, distribution, {
        labelFans: Number(state.label?.fans || 0),
        log: true
    });
    let dist = distResult.distribution;
    if (rush && dist !== "Digital") {
        logEvent("Digital rush applies only to Digital distribution. Using standard lead time.", "warn");
        rush = false;
    }
    const requestedAt = state.time.epochMs + scheduleHours * HOUR_MS;
    const scheduleWindow = resolveReleaseScheduleWindow(requestedAt, { distribution: dist, rush });
    const releaseAt = scheduleWindow.releaseAt;
    if (scheduleWindow.delayed) {
        logEvent(`Release window too soon for ${dist} lead time; auto-delayed to ${formatDate(releaseAt)}.`, "warn");
    }
    const feeResult = applyReleaseFees({ distribution: dist, rush, track });
    if (!feeResult.ok) {
        logEvent(`Cannot schedule release: insufficient funds for ${dist} release fees (${formatMoney(feeResult.total)}).`, "warn");
        return false;
    }
    const releaseNote = note || (rush ? "Digital Rush" : dist);
    state.releaseQueue.push({
        id: uid("RQ"),
        trackId: track.id,
        releaseAt,
        note: releaseNote,
    ...
    const feeParts = [];
    if (feeResult.distributionFee)
        feeParts.push(`Distribution fee: ${formatMoney(feeResult.distributionFee)}`);
    if (feeResult.rushFee)
        feeParts.push(`Rush fee: ${formatMoney(feeResult.rushFee)}`);
    const feeNote = feeParts.length ? ` ${feeParts.join(" ")}.` : "";
...
function scheduleReleaseAt(track, releaseAt, { distribution, note, rolloutMeta, suppressLog = false, rush = false } = {}) {
    if (!track || !track.actId || !getAct(track.actId)) {
    ...
    const scheduleWindow = resolveReleaseScheduleWindow(releaseAt, { distribution: dist, rush });
    const resolvedAt = scheduleWindow.releaseAt;
    if (scheduleWindow.delayed && !suppressLog) {
        logEvent(`Release window too soon for ${dist} lead time; auto-delayed to ${formatDate(resolvedAt)}.`, "warn");
    }
    const feeResult = applyReleaseFees({ distribution: dist, rush, track });
    if (!feeResult.ok) {
        return { ok: false, reason: `Insufficient funds for ${dist} release fees (${formatMoney(feeResult.total)}).`, fee: feeResult.total };
    }
    ...
    if (!suppressLog) {
        ...
        if (feeResult.rushFee)
            feeParts.push(`Rush fee: ${formatMoney(feeResult.rushFee)}`);
        const feeNote = feeParts.length ? ` ${feeParts.join(" ")}.` : "";
        logEvent(`Scheduled "${track.title}" for release on ${formatDate(resolvedAt)}.${feeNote}`);
    }
    if (typeof window !== "undefined") {
        window.dispatchEvent(new Event("rls:state-changed"));
    }
...
function buildTourDraftName({ act, eraId, anchorTrackIds, anchorProjectId } = {}) {
    ...
    if (era?.projectName)
        return `${era.projectName} Tour`;
    if (era?.name)
        return `${era.name} Tour`;
    const anchor = act?.id ? resolveTourAnchor({ anchorTrackIds, anchorProjectId }, act.id, eraId) : null;
    if (anchor?.projectName)
        return `${anchor.projectName} Tour`;
    if (act?.name)
        return `${act.name} Tour`;
    return null;
}
...
function resolveTourCooldownGap({ actId, draftId, scheduledAt }) {
    if (!actId || !Number.isFinite(scheduledAt))
        return null;
...
function autoGenerateTourDates({ draftId, count = null, startWeek = null, endWeek = null, filters = {} } = {}) {
    ...
    const hasTravelConflict = (scheduledAt, regionId) => {
        ...
        return listTourBookings({ actId: act.id })
            .concat(booked)
            .some((booking) => {
            if (!booking?.regionId || booking.regionId === regionId)
                return false;
        ...
        for (const day of dayOrder) {
            ...
            const selectBestVenue = (venueList) => {
                ...
                venueList.forEach((venue) => {
                    if (hasTravelConflict(scheduledAt, venue?.regionId))
                        return;
                    const validation = validateTourBooking({ draft, venue, scheduledAt });
                    if (!validation.ok)
                        return;
    ...
    if (resolvedBookings.length) {
        ...
        byTour.forEach((bookings, tourId) => {
            ...
            legs.forEach((leg, index) => {
                ...
                logEvent(`Tour leg recap (${tourId}): ${tourName} | ${legLabel} | Dates ${leg.count} | Attendance ${formatCount(leg.attendance)} | Net ${formatMoney(leg.profit)}.`);
            });
        });
    }
    return pending;
...
function applyChartWorkerResults(result, trackMap) {
    ...
    const globalScores = [];
    NATIONS.forEach((nation) => {
        const scored = Array.isArray(allScores.nations?.[nation]) ? allScores.nations[nation] : [];
        nationScores[nation] = scored.map((entry) => {
            const track = trackMap.get(entry.key);
            if (!track)
                return null;
            return { key: entry.key, track, score: entry.score, metrics: entry.metrics };
        }).filter(Boolean);
    });
    REGION_DEFS.forEach((region) => {
        const scored = Array.isArray(allScores.regions?.[region.id]) ? allScores.regions[region.id] : [];
        regionScores[region.id] = scored.map((entry) => {
            const track = trackMap.get(entry.key);
            if (!track)
                return null;
            return { key: entry.key, track, score: entry.score, metrics: entry.metrics };
    ...
    (result?.globalScores || []).forEach((entry) => {
        ...
        if (!track)
            return;
        globalScores.push({ key: entry.key, track, score: entry.score, metrics: entry.metrics });
    });
    applyDistributionAdjustments({ globalScores, nationScores, regionScores });
    const metricsByNation = {};
    const metricsByRegion = {};
    const globalMetricsByKey = new Map();
    NATIONS.forEach((nation) => {
        const map = new Map();
        nationScores[nation].forEach((entry) => map.set(entry.key, entry.metrics));
        metricsByNation[nation] = map;
    });
    REGION_DEFS.forEach((region) => {
        const map = new Map();
        regionScores[region.id].forEach((entry) => map.set(entry.key, entry.metrics));
        metricsByRegion[region.id] = map;
    });
    globalScores.forEach((entry) => globalMetricsByKey.set(entry.key, entry.metrics));
    NATIONS.forEach((nation) => {
        ...
        state.charts.nations[nation] = ordered.map((entry, index) => {
            ...
            return {
                ...
                track,
                score: entry.score,
                metrics: metricsByNation[nation]?.get(entry.key) || entry.metrics
            };
        }).filter(Boolean);
    });
    REGION_DEFS.forEach((region) => {
        ...
        state.charts.regions[region.id] = ordered.map((entry, index) => {
            ...
            return {
                ...
                track,
                score: entry.score,
                metrics: metricsByRegion[region.id]?.get(entry.key) || entry.metrics
            };
        }).filter(Boolean);
    });
...
function ensureTrendLedger() {
    ...
    if (!Array.isArray(state.trendLedger.rankings))
        state.trendLedger.rankings = [];
...
function getTrendRankHistory() {
    ...
    if (rankingSnapshots.length)
        return buildTrendRankHistoryFromRankings(rankingSnapshots);
...
function getTrendTopFiveStreak(genre, history) {
    ...
    for (let i = history.length - 1; i >= 0; i -= 1) {
        ...
        if (!entry?.ranks)
            break;
        if (getEmergingTrendWindow(genre, entry.week))
            break;
        const rank = entry.ranks[genre];
        if (!Number.isFinite(rank) || rank > TREND_PEAKING_TOP_RANK)
            break;
        streak += 1;
    }
    return streak;
...
function getTrendPeakingDisinterest(genre, history) {
    ...
    if (!streak || streak <= TREND_PEAKING_DISINTEREST_START)
        return 0;
...
function computeTrendDominanceDecay(streak, cap) {
    if (!Number.isFinite(streak) || !Number.isFinite(cap) || cap <= 1)
        return 1;
...
function applyTrendDominanceAdjustments(totals, ranking) {
    ...
    if (!topGenre)
        return { ranking, decayedTotals: { ...(totals || {}) }, resetGenre: null };
    const previous = ledger.dominance || {};
    let nextDominance = { ...previous };
    if (nextDominance.genre === topGenre) {
        if (!Number.isFinite(nextDominance.streak))
            nextDominance.streak = 0;
...
function resolveTrendStatusMeta(genre, history) {
    ...
    if (emerging)
        return { key: "EMERGING", label: "EMERGING", className: "" };
    if (!genre || !Array.isArray(history) || !history.length) {
    ...
    if (Number.isFinite(jump) && jump > 10)
        return { key: "HOT", label: "HOT", className: "warn" };
    if (Number.isFinite(jump) && jump > 5)
        return { key: "RISING", label: "RISING", className: "warn" };
    const drop = Number.isFinite(twoBack) && Number.isFinite(current) ? current - twoBack : null;
    if (Number.isFinite(drop) && drop >= 10)
        return { key: "FALLING", label: "FALLING", className: "danger" };
    let stable = false;
...
function addActPopularityPoints(yearEntry, actInfo, points, kind, week, seen) {
    ...
    if (kind === "tracks")
        entry.trackPoints += adjustedPoints;
    if (kind === "promos")
        entry.promoPoints += adjustedPoints;
    if (kind === "tours")
        entry.tourPoints += adjustedPoints;
    if (actInfo.isPlayer)
        entry.isPlayer = true;
    if (!seen.has(actKey)) {
        entry.weeksActive += 1;
...
function countAwardShowNominationsSince(startAt, labelName = state.label?.name) {
    if (!Number.isFinite(startAt) || !labelName)
        return 0;
    const store = ensureAwardShowStore();
    return (store.shows || []).reduce((total, show) => {
        if (!show || !Number.isFinite(show.showAt) || show.showAt < startAt)
            return total;
...
function buildPlanEligibleCategoriesFromStrategy(strategy, marketEntries, tracks) {
    ...
    if (strategy?.weeks) {
        strategy.weeks.forEach((week) => {
            (week?.events || []).forEach((eventItem) => {
                if (eventItem?.actionType)
                    promos.push(eventItem.actionType);
            });
        });
    }
    return {
...
function resolveRivalPromoCreatorIds(rival, market, entry = null) {
    ...
    if (fallbackCrew.length)
        return fallbackCrew.map((creator) => creator.id);
...
function normalizeAwardMetricStore(store) {
    if (!store || typeof store !== "object")
        return buildAwardMetricStore();
    if (!store.candidates || typeof store.candidates !== "object")
        store.candidates = {};
    if (!store.chartPoints || typeof store.chartPoints !== "object")
        store.chartPoints = {};
    if (!store.sales || typeof store.sales !== "object")
        store.sales = {};
    if (!store.critics || typeof store.critics !== "object")
        store.critics = {};
    if (!store.awards || typeof store.awards !== "object")
        store.awards = {};
    if (!store.firstLeadWeek || typeof store.firstLeadWeek !== "object")
        store.firstLeadWeek = {};
...
function ensureAnnualAwardLedgerYear(year) {
    ...
    if (typeof entry.totalWeeks !== "number")
        entry.totalWeeks = 0;
    if (typeof entry.lastUpdateWeek !== "number")
        entry.lastUpdateWeek = null;
    if (typeof entry.lastUpdatedAt !== "number")
...
function resolveMarketEntryById(marketId) {
    if (!marketId)
        return null;
    const active = getMarketTrackById(marketId);
    if (active)
        return active;
    return getArchivedMarketTrackById(marketId) || null;
}
function resolveMarketEntryByTrackId(trackId) {
    if (!trackId)
        return null;
    const active = getMarketTrackByTrackId(trackId);
    if (active)
        return active;
    return getArchivedMarketTrackByTrackId(trackId) || null;
}
...
async function maybeReleaseAnnualAwards(now = state.time.epochMs) {
    ...
    if (targetYear < 1)
        return null;
    if (hasAnnualAwardsForYear(targetYear, { requireComplete: true }))
        return null;
...
function resolveAwardShowScheduleAt(year, schedule) {
    if (!Number.isFinite(year) || !schedule)
        return null;
...
function awardShowTimeline(showAt) {
    if (!Number.isFinite(showAt))
        return { lockAt: null, revealAt: null, weekStart: null };
...
function normalizeAwardShowCategory(category) {
    ...
    if (typeof next.label !== "string")
        next.label = next.label || "Award Category";
    if (!next.contentType)
        next.contentType = "tracks";
    if (!next.ratingMode)
        next.ratingMode = "critics";
    if (!Number.isFinite(next.nomineeCount))
        next.nomineeCount = 6;
    if (!Array.isArray(next.nominees))
        next.nominees = [];
    if (!next.winner || typeof next.winner !== "object")
        next.winner = null;
    if (typeof next.scoreKey !== "string")
        next.scoreKey = null;
    if (!Number.isFinite(next.windowWeeks))
        next.windowWeeks = AWARD_SHOW_WINDOW_WEEKS;
    if (typeof next.lastUpdatedAt !== "number")
...
function normalizeAwardShowPerformance(performance) {
    ...
    if (!next.id)
        next.id = uid("ASP");
    if (typeof next.label !== "string")
        next.label = next.label || "Performance";
    ...
    if (typeof next.labelName !== "string")
...
function normalizeAwardShowEntry(show) {
    ...
    if (typeof next.familyId !== "string")
        next.familyId = next.familyId || null;
    if (typeof next.label !== "string")
        next.label = next.label || "Award Show";
    if (typeof next.ratingFocus !== "string")
        next.ratingFocus = next.ratingFocus || null;
    if (!Number.isFinite(next.showAt))
        next.showAt = null;
    if (!Number.isFinite(next.nominationLockAt))
        next.nominationLockAt = null;
    if (!Number.isFinite(next.nominationRevealAt))
        next.nominationRevealAt = null;
    if (!next.nominationWindow || typeof next.nominationWindow !== "object") {
        ...
        if (!Number.isFinite(window.startWeek))
            window.startWeek = null;
        if (!Number.isFinite(window.endWeek))
            window.endWeek = null;
        if (!Number.isFinite(window.startEpoch))
            window.startEpoch = null;
        if (!Number.isFinite(window.endEpoch))
            window.endEpoch = null;
    }
    if (typeof next.status !== "string")
    ...
    if (typeof next.monthKey !== "string")
        next.monthKey = next.monthKey || null;
    if (!Number.isFinite(next.year))
        next.year = null;
    if (!Number.isFinite(next.monthIndex))
        next.monthIndex = null;
    if (typeof next.lockedAt !== "number")
        next.lockedAt = null;
    if (typeof next.revealedAt !== "number")
        next.revealedAt = null;
    if (typeof next.resolvedAt !== "number")
        next.resolvedAt = null;
    if (typeof next.createdAt !== "number")
        next.createdAt = null;
    if (typeof next.lockNote !== "string")
        next.lockNote = next.lockNote || null;
    return next;
}
function ensureAwardShowStore() {
    ...
    if (!Array.isArray(state.meta.awardShows.shows))
        state.meta.awardShows.shows = [];
...
function applyAwardBoostEntry(map, key, pct, weeks, meta) {
    if (!key || !map || !Number.isFinite(pct) || pct <= 0)
        return false;
    const now = state.time.epochMs;
...
function resolveAwardTrackReleaseAt(entry) {
    if (!entry)
        return null;
    const market = entry.marketId ? resolveMarketEntryById(entry.marketId) : null;
    if (Number.isFinite(market?.releasedAt))
        return market.releasedAt;
    const track = entry.trackId ? getTrack(entry.trackId) : null;
    if (Number.isFinite(track?.releasedAt))
        return track.releasedAt;
    return null;
}
...
function ensureShowCandidate(map, key, entry, contentType) {
    ...
    if (!candidate.projectName && entry?.projectName)
        candidate.projectName = entry.projectName;
    if (entry?.projectType)
        candidate.projectType = normalizeProjectType(entry.projectType || candidate.projectType);
    if (!candidate.trackId && entry?.trackId)
        candidate.trackId = entry.trackId;
    if (!candidate.marketId && entry?.marketId)
        candidate.marketId = entry.marketId;
    if (!candidate.primaryTrackId && entry?.primaryTrackId)
        candidate.primaryTrackId = entry.primaryTrackId;
    return candidate;
}
...
function buildAwardCandidateMaps(snapshots, window) {
    ...
    list.forEach((snapshot) => {
            ...
            entries.forEach((entry) => {
                ...
                if (promoMeta) {
                    ...
                    if (!Number.isFinite(candidate.createdAt))
                        candidate.createdAt = promoMeta.createdAt;
                }
        ...
        entries.forEach((entry) => {
            ...
            if (!Number.isFinite(candidate.releasedAt))
                candidate.releasedAt = resolveAwardTrackReleaseAt(entry);
            const criticScore = resolveCriticScoreForEntry(entry, scopeKey);
        ...
        projectEntries.forEach((entry) => {
            ...
            if (!Number.isFinite(candidate.releasedAt))
                candidate.releasedAt = resolveAwardProjectReleaseAt(entry);
            if (entry.primaryTrackId)
                candidate.primaryTrackId = entry.primaryTrackId;
            const criticScore = Number(entry.criticScore || 0);
            const audienceScore = salesEquivalentFromMetrics(entry.metrics);
            candidate.criticScoreTotal += criticScore * weight;
...
function buildActPromoTypeMap(window) {
    const map = new Map();
    if (!window || !Array.isArray(state.promoContent))
        return map;
    state.promoContent.forEach((entry) => {
        if (!entry || !Number.isFinite(entry.createdAt))
            return;
        if (!isEpochInWindow(entry.createdAt, window))
            return;
        const actKey = resolveAwardActKey(entry);
        if (!actKey)
            return;
...
function buildEligibleActSet(maps, window) {
    ...
    const addAct = (candidate) => {
        if (candidate?.actKey)
            set.add(candidate.actKey);
    };
    Array.from(maps.tracks.values()).forEach((candidate) => {
        if (Number.isFinite(candidate.releasedAt) && isEpochInWindow(candidate.releasedAt, window))
            addAct(candidate);
    });
    ...
    Array.from(maps.promos.values()).forEach((candidate) => {
        if (Number.isFinite(candidate.createdAt) && isEpochInWindow(candidate.createdAt, window))
            addAct(candidate);
    });
    Array.from(maps.tours.values()).forEach((candidate) => {
        if (candidate.dateCount >= 2)
            addAct(candidate);
    });
    return set;
}
function isEpochInWindow(epochMs, window) {
    if (!Number.isFinite(epochMs) || !window)
        return false;
...
function applyHybridScores(candidates) {
    ...
    candidates.forEach((candidate) => {
        ...
        const hybridScore = Math.round((candidate.criticScore || 0) * AWARD_HYBRID_CRITIC_WEIGHT
...
function resolveActFirstReleaseAt(candidate) {
    ...
    const pool = [];
    if (Array.isArray(state.marketTracks))
        pool.push(...state.marketTracks);
    if (Array.isArray(state.meta?.marketTrackArchive))
        pool.push(...state.meta.marketTrackArchive);
    const stamps = pool
        .filter((track) => {
        if (!Number.isFinite(track?.releasedAt))
            return false;
        if (actId && track.actId && track.actId !== actId)
            return false;
        if (!actId && actName && track.actName && track.actName !== actName)
            return false;
        if (label && track.label && track.label !== label)
            return false;
        return true;
    })
...
function isBreakthroughAct(candidate, showAt) {
    if (!candidate || !Number.isFinite(showAt))
        return false;
    const firstReleaseAt = resolveActFirstReleaseAt(candidate);
    if (!Number.isFinite(firstReleaseAt))
        return false;
...
function buildAwardNomineePool(category, maps, window, actPromoTypes, eligibleActs, showAt) {
    if (!category || !maps)
        return [];
    let pool = [];
    if (category.contentType === "tracks")
        pool = Array.from(maps.tracks.values());
    else if (category.contentType === "projects")
        pool = Array.from(maps.projects.values());
    else if (category.contentType === "promos")
        pool = Array.from(maps.promos.values());
    else if (category.contentType === "tours")
        pool = Array.from(maps.tours.values());
    else if (category.contentType === "acts")
        pool = Array.from(maps.acts.values());
    pool = pool.filter(Boolean);
    applyHybridScores(pool);
    if (category.contentType === "tracks" || category.contentType === "projects") {
        pool = pool.filter((candidate) => (Number.isFinite(candidate.releasedAt)
...
function resolveAwardCandidateTimestamp(candidate) {
    if (!candidate)
        return 0;
    if (Number.isFinite(candidate.releasedAt))
        return candidate.releasedAt;
    if (Number.isFinite(candidate.createdAt))
        return candidate.createdAt;
    if (Number.isFinite(candidate.firstSeenAt))
...
function resolveAwardWinner(category) {
    if (!category || !Array.isArray(category.nominees))
        return null;
...
async function lockAwardShowNominations(show) {
    if (!show || !Number.isFinite(show.showAt))
        return { ok: false, reason: "invalid show" };
...
function applyQuarterHourResourceTick(activeOrders = [], dayIndex = null) {
    ...
    let regenCount = 0;
    state.creators.forEach((creator) => {
        if (hasBusyCreators && busyIds.has(creator.id))
            return;
...
async function advanceQuarters(quarters, { renderQuarterly = true, renderAfter = !renderQuarterly } = {}) {
    ...
    advanceQuartersQueue = advanceQuartersQueue.then(async () => {
        if (state.meta.gameOver)
            return;
        if (suppressWeeklyRender)
            session.suppressWeeklyRender = true;
        try {
            ...
            if (suppressWeeklyRender)
                session.suppressWeeklyRender = false;
        }
        if (renderAfter) {
            uiHooks.renderAll?.({ save: false });
        }
    ...
    if (secPerQuarter !== Infinity) {
        ...
        if (queuedIterations > QUARTER_TICK_FRAME_LIMIT) {
            ...
            if (warnNow - session.lastPerfThrottleAt >= PERF_THROTTLE_LOG_COOLDOWN_MS) {
                ...
                console.warn(`[perf] tick backlog ${queuedIterations} quarter-hour iterations; throttling to ${QUARTER_TICK_FRAME_LIMIT} per frame.`);
...
function normalizeState() {
    ...
    if (!state.ui.trendScopeTarget)
        state.ui.trendScopeTarget = defaultTrendNation();
    if (typeof state.ui.rivalRosterId === "undefined")
        state.ui.rivalRosterId = null;
    if (typeof state.ui.rivalRosterFocus !== "boolean")
    ...
    if (!state.ui.quickActFilters || typeof state.ui.quickActFilters !== "object") {
        ...
        if (!["2-3", "2", "3"].includes(filters.groupSize))
            filters.groupSize = "2-3";
        ...
        if (typeof filters.ageGroup !== "string")
            filters.ageGroup = "";
        if (filters.theme !== "Any" && !THEMES.includes(filters.theme))
            filters.theme = "Any";
        if (filters.mood !== "Any" && !MOODS.includes(filters.mood))
            filters.mood = "Any";
        if (filters.alignment !== "Label" && !ALIGNMENTS.includes(filters.alignment))
            filters.alignment = "Label";
    ...
    state.rolloutStrategies = state.rolloutStrategies.filter(Boolean).map((strategy) => {
        ...
        if (typeof next.focusType !== "string")
            next.focusType = era ? "Era" : "Campaign";
        if (typeof next.focusId !== "string")
            next.focusId = era?.id || null;
        if (typeof next.focusLabel !== "string")
            next.focusLabel = era?.name || next.label || null;
    ...
    state.rivalReleaseQueue = state.rivalReleaseQueue.filter(Boolean).map((entry) => {
        ...
        if (typeof next.actNameKey !== "string")
            next.actNameKey = next.actNameKey || null;
        if (typeof next.huskId !== "string")
            next.huskId = next.huskId || null;
        if (typeof next.huskSource !== "string")
            next.huskSource = next.huskSource || null;
        if (typeof next.focusType !== "string")
            next.focusType = next.focusType || null;
        if (typeof next.focusId !== "string")
            next.focusId = next.focusId || null;
        if (typeof next.focusLabel !== "string")
            next.focusLabel = next.focusLabel || null;
    ...
    if (state.tracks?.length) {
        state.tracks.forEach((track) => {
            ...
            if (!Number.isFinite(track.trendRankAtRelease))
                track.trendRankAtRelease = null;
            if (!Number.isFinite(track.trendTotalAtRelease))
                track.trendTotalAtRelease = null;
            if (typeof track.rolloutStrategyId !== "string")
                track.rolloutStrategyId = track.rolloutStrategyId || null;
            if (typeof track.rolloutItemId !== "string")
                track.rolloutItemId = track.rolloutItemId || null;
            if (!Number.isFinite(track.rolloutWeekIndex))
                track.rolloutWeekIndex = null;
    ...
    if (state.marketTracks?.length) {
        state.marketTracks.forEach((entry) => {
            ...
            if (!Number.isFinite(entry.trendRankAtRelease))
            ...
            if (typeof entry.rolloutPlanSource !== "string")
                entry.rolloutPlanSource = entry.rolloutPlanSource || null;
            if (typeof entry.rolloutFocusType !== "string")
                entry.rolloutFocusType = entry.rolloutFocusType || null;
            if (typeof entry.rolloutFocusId !== "string")
                entry.rolloutFocusId = entry.rolloutFocusId || null;
            if (typeof entry.rolloutFocusLabel !== "string")
                entry.rolloutFocusLabel = entry.rolloutFocusLabel || null;
...
function recommendReleasePlan(track) {
    ...
    const config = resolveReleaseDistributionConfig();
    const leadDigital = config.digitalLeadWeeks * WEEK_HOURS;
    const leadPhysical = config.physicalLeadWeeks * WEEK_HOURS;
    const isPlayer = typeof track?.isPlayer === "boolean" ? track.isPlayer : Boolean(track?.actId);
    const labelFans = isPlayer ? Number(state.label?.fans || 0) : null;
    const eligibility = evaluatePhysicalEligibility(track, { labelFans });
    if (trendMatch) {
        return {
            ...
            scheduleHours: 0,
            rush: true,
            reason: "Trend match favors a rushed digital release."
        };
    }
    if (eligibility.ok) {
        return {
            ...
            scheduleHours: leadPhysical,
            rush: false,
            reason: "Physical rollout unlocked by quality and fan thresholds."
        };
    }
    return {
        version: RECOMMEND_VERSION,
        distribution: "Digital",
        scheduleKey: "week",
        scheduleHours: leadDigital,
        rush: false,
        reason: "Give one week for digital prep and momentum."
    };
}
function planRivalRelease(genre, quality) {
    ...
    return {
        ...
        releaseAt: getReleaseAsapAtForDistribution(plan.distribution, { rush: plan.rush })
    };
}
