...
function updateTourDraft(draftId, updates = {}) {
    ...
    if (TOUR_GOALS.includes(updates.goal))
        draft.goal = updates.goal;
...
function applyTourVisibilityBoost(booking) {
    if (!booking || booking.goal !== "visibility")
        return;
...
function resolveChartEntryAct(entry) {
    if (!entry)
        return { actId: null, actName: "", label: "", country: "", isPlayer: false };
    const base = entry.track || entry;
...
function normalizeState() {
    ...
    touring.drafts = touring.drafts.filter(Boolean).map((draft) => {
        ...
        if (typeof next.name !== "string")
            next.name = "";
        if (next.goal !== "visibility" && next.goal !== "revenue")
            next.goal = "visibility";
