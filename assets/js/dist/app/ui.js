...
function resolvePromoScheduleEpochMs(week, dayIndex, timeframeId) {
    if (!Number.isFinite(week) || !Number.isFinite(dayIndex) || !timeframeId)
        return null;
...
function closeCreatorActsPopovers(exceptId = null) {
    document.querySelectorAll("[data-creator-acts-popover]").forEach((popover) => {
        ...
        if (exceptId && popoverId === exceptId)
            return;
...
function bindGlobalHandlers() {
    ...
    document.addEventListener("click", (event) => {
        ...
        if (target.closest("[data-creator-acts-popover]"))
            return;
        if (target.closest("[data-creator-acts-more]"))
            return;
...
function bindViewHandlers(route, root) {
    ...
    if (route === "releases") {
        ...
        const syncFooter = () => {
            ...
            if (footerPanel)
                footerPanel.classList.toggle("is-collapsed", collapsed);
    ...
    if (dashboardFocusTabs) {
        dashboardFocusTabs.addEventListener("click", (e) => {
            ...
            if (next === state.ui.dashboardFocusPanel)
                return;
    ...
    if (creatorList) {
        creatorList.addEventListener("click", (e) => {
            ...
            if (e.target.closest("[data-creator-acts-popover]"))
                return;
...
function buildPromoOverlapDebugSnapshot() {
    ...
    (state.releaseQueue || []).forEach((entry) => {
        if (!entry?.trackId || !Number.isFinite(entry.releaseAt))
            return;
    ...
    const findProjectTracks = (projectSpec) => {
        ...
        return state.tracks.filter((track) => {
            if (!track)
                return false;
            if (projectSpec.actId && track.actId !== projectSpec.actId)
                return false;
            if (projectSpec.eraId && track.eraId !== projectSpec.eraId)
                return false;
            const trackProject = track.projectName || `${track.title} - Single`;
            if (normalizeProjectName(trackProject) !== projectName)
                return false;
    ...
    bankedTracks.forEach((entry) => {
        ...
        if (entry.status === "released" || entry.status === "shelved")
    ...
    const resolveTargetType = (entry) => {
        ...
        if (entry?.contentId && parsePromoProjectKey(entry.contentId))
            return "project";
        if (entry?.contentId)
            return "track";
        return "act";
    ...
    state.rivalReleaseQueue.forEach((entry) => {
        if (!entry || (entry.queueType || "release") !== "promo")
            return;
        const hasUpcomingRelease = state.rivalReleaseQueue.some((releaseEntry) => {
            if (!releaseEntry || (releaseEntry.queueType || "release") !== "release")
                return false;
            if (releaseEntry.label !== entry.label)
                return false;
...
function exportDebugBundle() {
    ...
    downloadFile("promo_overlap_snapshot.json", JSON.stringify(buildPromoOverlapDebugSnapshot(), null, 2), "application/json");
...
function runPromotion() {
    ...
    for (const promoType of effectiveTypes) {
        ...
        if (!facilityId)
            continue;
        const reservation = reservePromoFacilitySlot(facilityId, promoType, trackContext.track?.id || null, { actId: act.id, epochMs: availabilityEpoch });
        if (!reservation.ok) {
