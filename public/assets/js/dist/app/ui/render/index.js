...
function renderRolloutAnalyticsTable({ title, headers, rows, emptyMessage = "No data yet." } = {}) {
...
function computeLabelShare(entries, size, labelName) {
    if (!Array.isArray(entries) || !labelName || !Number.isFinite(size) || size <= 0)
        return null;
    let count = 0;
    entries.forEach((entry) => {
        if (resolveChartEntryLabel(entry) === labelName)
            count += 1;
    });
    if (!count)
        return null;
...
function maybeWarnMonopolyRisk(share, label) {
    if (!label || !Number.isFinite(share))
        return;
    ...
    if (share >= warningThreshold) {
        if (!state.ui.monopolyWarningActive) {
            ...
            showToast("You are at risk of Monopoly Loss.", `Your label controls ${Math.round(share * 100)}% of ${label}.`, { tone: "warn" });
        }
    }
...
function updateMonopolyRiskIndicator() {
    ...
    if (!best) {
        if (statusEl)
            statusEl.textContent = `0% / ${thresholdPct}%`;
        if (meter)
            meter.style.width = "0%";
        if (note)
            note.textContent = `Threshold ${thresholdPct}%`;
        maybeWarnMonopolyRisk(0, "");
        return;
    }
    const sharePct = Math.round(best.share * 100);
    if (statusEl)
        statusEl.textContent = `${sharePct}% / ${thresholdPct}% (${best.label})`;
    if (meter)
        meter.style.width = `${Math.min(100, sharePct)}%`;
    if (note)
        note.textContent = `Threshold ${thresholdPct}% (${best.label})`;
...
function renderPromoAlerts() {
    ...
    const trackAlerts = tracks.map((track) => {
        ...
        const missing = PROMO_TRACK_REQUIRED_TYPES.filter((typeId) => {
            if (typeId === "musicVideo" && !isTrackReleaseReleased(track))
                return false;
...
async function updateSaveStatusPanel(root) {
    ...
    if (targetDetail)
        targetDetail.textContent = withLocalIssue("External storage not configured.");
    ...
    try {
        ...
        if (status?.supported === false) {
            if (targetDetail)
                targetDetail.textContent = withLocalIssue("External storage not supported in this browser.");
        }
        else if (status?.status === "ready") {
            if (targetLabel)
                targetLabel.textContent = `Save target: ${status.name || "External folder"}`;
            if (targetDetail)
                targetDetail.textContent = withLocalIssue("External storage ready.");
            externalReady = true;
        }
        else if (status?.status === "not-set") {
            if (targetDetail)
                targetDetail.textContent = withLocalIssue("External folder not selected.");
        }
        else if (status?.status) {
            if (targetDetail)
                targetDetail.textContent = withLocalIssue(`External storage permission: ${status.status}.`);
        }
    }
    catch {
        if (targetDetail)
            targetDetail.textContent = withLocalIssue("Unable to read external storage status.");
    }
    if (saveBtn) {
...
function renderActs() {
    ...
    const renderActCard = (act) => {
        ...
        readiness.projects.forEach((project) => {
            if (!topProject || project.count > topProject.count)
                topProject = project;
        });
...
function countAnnualAwardsForLabel(labelName, year) {
    if (!labelName || !Number.isFinite(year))
        return 0;
    const list = Array.isArray(state.meta?.annualAwards) ? state.meta.annualAwards : [];
    const entry = list.find((award) => award?.year === year);
    if (!entry || !entry.awards)
        return 0;
...
function buildPlayerEraSummary(labelName) {
    ...
    state.tracks.forEach((track) => {
        if (!track || track.eraId !== era.id)
            return;
        const economy = track.economy || {};
        if (!Number.isFinite(economy.chartPoints))
            return;
        pointsSum += economy.chartPoints;
        pointsSeen = true;
    });
    if (pointsSeen)
        chartPoints = Math.round(pointsSum);
...
function buildRivalEraSummary(rival) {
    if (!rival?.name)
        return { label: "Rival", name: "Rival era", hasData: false };
    ...
    windowReleases.forEach((entry) => {
        ...
        if (!Number.isFinite(peak) || !Number.isFinite(weeks))
            return;
...
function renderRivalKpiBlock({ title, subtitle, kpi, consumption, isPlayer }) {
    ...
    if (consumption?.hasMetrics) {
        ...
        if (sales !== "-")
            parts.push(`Sales ${sales}`);
        if (streaming !== "-")
            parts.push(`Streaming ${streaming}`);
        if (airplay !== "-")
            parts.push(`Airplay ${airplay}`);
        if (social !== "-")
            parts.push(`Social ${social}`);
...
function renderTrackRolloutToggleTags(toggles) {
    ...
    const tags = [];
    if (flags.musicVideoOn)
        tags.push({ label: "Video On" });
    if (flags.tourTieInOn)
        tags.push({ label: "Tour tie-in On" });
    if (flags.primeTimeLiveOn)
        tags.push({ label: "Prime time On" });
...
function renderTrackRolloutStrategy() {
    ...
    if (!trackSelect || !templateList || !details)
        return;
    ...
    if (!tracks.length) {
        ...
        trackSelect.disabled = true;
        if (trackMeta)
            trackMeta.textContent = "No tracks available.";
...
function collectProjectSummaries(tracks, queuedIds) {
    ...
    tracks.forEach((track) => {
        ...
        if (isTrackReleaseScheduled(track) || queuedIds.has(track.id))
...
function renderReleaseDesk() {
    ...
    const isReleaseReadyTrack = (track) => (Boolean(track)
        && (track.status === "Ready"
    ...
    if (crewSummaryList || crewStaminaList) {
        if (!draft) {
            ...
            if (!crew.length) {
                ...
                const maxDates = crew.map((creator) => (estimateCreatorMaxConsecutiveTourDates(creator, crewCount, { boostPct: staffing.boostPct })));
...
function getBlockedAchievementReason() {
    if (state.meta?.cheaterMode)
        return "Cheater mode active: achievements pause until disabled.";
    if (state.meta?.monopolyLoss)
        return "Monopoly loss: achievements are blocked this run.";
    if (state.meta?.achievementsLocked && !state.meta?.bailoutUsed)
        return "Achievements locked until bailout clears.";
    return "";
}
...
function updateAchievementSummaries() {
    ...
    if (dashboardSummary)
        dashboardSummary.textContent = summaryText;
...
function shouldThrottleWorldRender() {
    ...
    if (isFast && Number.isFinite(totalHours) && Number.isFinite(lastWorldRenderHour)) {
        if (totalHours - lastWorldRenderHour < WORLD_RENDER_FAST_HOUR_STEP)
            return true;
    }
...
function recordWorldRender() {
    ...
    if (Number.isFinite(state.time?.totalHours))
        lastWorldRenderHour = state.time.totalHours;
    if (state.ui?.forceWorldRender)
...
function shouldThrottleChartsRender() {
    if (state.time?.speed === "pause")
        return false;
    if (state.ui?.forceChartsRender)
        return false;
    const now = Date.now();
    const totalHours = Number.isFinite(state.time?.totalHours) ? state.time.totalHours : null;
    const isFast = state.time?.speed === "fast" || session.timeJumpActive;
    if (isFast && Number.isFinite(totalHours) && Number.isFinite(lastChartsRenderHour)) {
        if (totalHours - lastChartsRenderHour < CHART_RENDER_FAST_HOUR_STEP)
            return true;
    }
...
function recordChartsRender() {
    ...
    if (Number.isFinite(state.time?.totalHours))
        lastChartsRenderHour = state.time.totalHours;
    if (state.ui?.forceChartsRender)
        state.ui.forceChartsRender = false;
}
function safeRenderCreateView() {
    try {
        ...
        showToast("Create view error", "The Create tab encountered an issue; try reloading the view or switching tabs to recover.", { tone: "warn" });
    }
}
...
export { refreshSelectOptions, updateActMemberFields, renderAutoAssignModal, renderTime, renderStats, renderSlots, renderActs, renderCreators, renderEraStatus, renderTracks, renderModifierInventory, renderReleaseDesk, renderTrackRolloutStrategy, renderTouringDesk, renderQuickRecipes, renderCalendarView, renderCalendarList, renderCalendarDayDetail, renderGenreIndex, renderCommunityRankings, renderStudiosList, renderRoleActions, renderTutorialEconomy, renderModifierTools, renderCharts, renderWallet, renderLossArchives, renderResourceTickSummary, renderAwardsCircuit, renderSocialFeed, renderMainMenu, updateSaveStatusPanel, renderRankingModal, renderRankingWindow, renderAll, renderActiveStudiosSelect, renderCreateStageTrackSelect, renderCreateStageControls, renderActiveView, renderMarket, renderRivalRosterPanel, renderEventLog, renderSystemLog, renderTrends, renderCommunityLabels, renderTopBar, renderPopulation, renderEconomySummary, renderActiveCampaigns, renderInventory, renderWorkOrders, renderTrackHistoryPanel, renderRolloutStrategyPlanner, renderCreateTrends, renderAchievements, renderQuests, renderActiveArea, renderCalendarEraList, renderCreatorFallbackSymbols, renderCreatorAvatar, openMainMenu, closeMainMenu, updateGenrePreview, };
