...
function buildSessionPayload(session) {
    if (!session)
        return { payload: null, bytes: 0, trimmed: false };
    let payload = JSON.stringify(session);
    let bytes = estimatePayloadBytes(payload);
    if (bytes <= SESSION_STORAGE_MAX_BYTES)
        return { payload, bytes, trimmed: false };
    const trimmed = {
        ...session,
...
function appendEvent(entry, { isAction = false, reportToConsole = false, level = "info" } = {}) {
    ...
    if (currentSession.events.length > EVENT_LIMIT)
    ...
    if (isAction) {
        ...
        if (actionTrail.length > TRAIL_LIMIT)
            actionTrail.shift();
...
export function recordUsageError({ kind = "error", message = "", stack = null, filename = null, lineno = null, colno = null, reason = null, route = null, gameTs = null, reportToConsole = true } = {}) {
    ensureSession();
    const ts = nowTs();
    const entry = {
        ts,
    ...
    if (currentSession.errors.length > ERROR_LIMIT)
        currentSession.errors.shift();
