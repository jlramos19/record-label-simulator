import { ACT_NAME_ADJECTIVE_CATEGORY_IDS, ACT_NAME_NOUN_CATEGORY_IDS, ACT_NAME_CATEGORIES, ACT_NAME_TOTALS } from "./act-name-pools.js";
export const ACT_NAME_KEY_SEPARATOR = "::";
export const ACT_NAME_NATION_BIAS_WEIGHTS = {
    annglora: {
        adjective_weights: {
            colors_basic: 2,
            light_glow: 3,
...
const resolveRng = ({ rng, seed } = {}) => {
    if (typeof rng === "function")
        return rng;
    const resolvedSeed = resolveSeed(seed);
    return Number.isFinite(resolvedSeed) ? makeSeededRng(resolvedSeed) : Math.random;
};
const normalizeActKind = (actKind) => actKind === "solo" ? "solo" : "group";
const normalizeNationKey = (nation) => String(nation || "annglora").toLowerCase();
const resolveNationWeights = (nation, override) => {
    if (override)
        return override;
    const key = normalizeNationKey(nation);
    return ACT_NAME_NATION_BIAS_WEIGHTS[key] || ACT_NAME_NATION_BIAS_WEIGHTS.annglora;
};
const resolveCategoryWeight = (categoryId, baseWeights, actKindOverrides, actKind) => {
...
const pickWeightedCategory = (categoryIds, weights, actKindOverrides, actKind, rng) => {
...
const seededPick = (list, rng) => list[Math.floor(rng() * list.length)];
export const makeActNameKey = (adjectiveId, nounId) => `${adjectiveId}${ACT_NAME_KEY_SEPARATOR}${nounId}`;
export const parseActNameKey = (nameKey) => {
    ...
    if (!adjectiveId || !nounId)
        return null;
    return { adjectiveId, nounId };
};
export const generateActNamePair = ({ nation = "Annglora", actKind = "group", rng, seed, adjectiveCategoryId, nounCategoryId, weights } = {}) => {
...
export const generateUniqueActNamePairs = ({ count = 1, nation = "Annglora", actKind = "group", rng, seed, existingKeys = [], maxAttempts } = {}) => {
    const rngFn = resolveRng({ rng, seed });
    const resolvedActKind = normalizeActKind(actKind);
...
//# sourceMappingURL=act-name-generator.js.map