...
function renderPlanCadenceLine(plan) {
  ...
  if (releases.length) parts.push(`Releases: ${releases.join(", ")}`);
  if (promoLabels.length) parts.push(`Promos: ${promoLabels.join(", ")}`);
...
function renderPlanAdoptionOdds(planId, oddsByPlanId) {
  ...
  if (!entries.length) return `<div class="muted">Adoption odds: No rival labels available.</div>`;
...
function renderCharts() {
  ...
  if (chartTabs) chartTabs.classList.toggle("hidden", isActs || isRollouts);
  ...
  if (isRollouts) {
    ...
    if ($("chartWeekRange")) $("chartWeekRange").textContent = "Rollout strategies";
    if ($("chartHistoryNotice")) $("chartHistoryNotice").textContent = "";
    const meta = $("chartMeta");
    if (meta) meta.textContent = "Plan usage, outcomes, and rival adoption odds.";
...
function formatWinLeaderboardPoints(value) {
  ...
  if (Math.abs(rounded % 1) < 0.05) return formatCount(rounded);
...
function renderWinLeaderboard() {
  ...
  if (!entries.length) {
    if (metaEl) metaEl.textContent = "No wins recorded yet.";
