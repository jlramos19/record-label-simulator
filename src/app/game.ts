...
function refreshPerfTimingConfig() {
  ...
  const now = nowMs();
  if (now - perfTiming.lastConfigAt < 1000) return;
  ...
  if (overrides && typeof overrides === "object") {
    if (typeof overrides.enabled === "boolean") enabled = overrides.enabled;
...
function shouldSamplePerfTiming() {
  if (!perfTiming.enabled || !canUseUserTiming()) return false;
...
function ensureDistributionStore() {
  ...
  if (typeof state.distribution.pendingUsageWeek !== "number") state.distribution.pendingUsageWeek = null;
  if (typeof state.distribution.lastAppliedWeek !== "number") state.distribution.lastAppliedWeek = null;
  if (typeof state.distribution.lastWarningWeek !== "number") state.distribution.lastWarningWeek = null;
...
function resolveRegionPopulation(snapshot, regionId) {
  ...
  if (lowerId.includes("capital")) return nation.capital || 0;
  if (lowerId.includes("elsewhere")) return nation.elsewhere || 0;
...
function buildDistributionSnapshot() {
  ...
  chunks.forEach((chunk) => {
    ...
    if (!regionId || !regionSample[regionId]) return;
...
function logDistributionCapacityWarnings(usageByRegion) {
  ...
  if (store.lastWarningWeek === week) return;
  ...
  logEvent(
    `Physical demand exceeded store capacity in ${constrained.length} region${constrained.length === 1 ? "" : "s"} (top: ${topLabel}).`,
    "warn"
  );
...
function applyDistributionAdjustments({ globalScores, nationScores, regionScores } = {}) {
  ...
  REGION_DEFS.forEach((region) => {
    ...
    usageByRegion[region.id] = applyDistributionAdjustmentsForScope(
      entries,
      {
  ...
  NATIONS.forEach((nation) => {
    ...
    nationUsage[nation] = applyDistributionAdjustmentsForScope(
      entries,
      {
  ...
  const globalUsage = applyDistributionAdjustmentsForScope(
    globalScores || [],
    {
...
function applyDistributionInventoryForWeek() {
  const store = ensureDistributionStore();
  const week = weekIndex() + 1;
  if (store.lastAppliedWeek === week) return false;
  const snapshot = buildDistributionSnapshot();
...
function normalizeCreator(creator) {
  ...
  if (typeof creator.hasReleaseHistory !== "boolean") creator.hasReleaseHistory = Number.isFinite(creator.lastReleaseAt);
  if (typeof creator.hasPromoHistory !== "boolean") creator.hasPromoHistory = Number.isFinite(creator.lastPromoAt);
  if (typeof creator.memorializedAt !== "number") creator.memorializedAt = null;
  if (typeof creator.memorializedReason !== "string") creator.memorializedReason = null;
...
function memorializeCreator(creator, { reason = "age", atMs = state.time.epochMs } = {}) {
  if (!creator || Number.isFinite(creator.memorializedAt)) return false;
  ...
  if (!state.meta) state.meta = makeDefaultState().meta;
  if (!Array.isArray(state.meta.memorializedCreators)) state.meta.memorializedCreators = [];
...
function advanceCreatorAgesYear(year) {
  ...
  const applyAge = (creator) => {
    if (!creator || Number.isFinite(creator.memorializedAt)) return;
...
function setActStatus(act, status, reason = null, { log = true } = {}) {
  ...
  if (prev === next && act.statusReason === reason) return act;
  act.status = next;
...
function actHasSignedMembers(act) {
  if (!act || !Array.isArray(act.memberIds)) return false;
...
function legacySoloActsForCreator(creatorId, activeActId, { reason = "Legacy" } = {}) {
  ...
  state.acts.forEach((act) => {
    if (!act || act.id === activeActId) return;
    const actKind = resolveActKindFromType(act.type);
    if (actKind !== "solo") return;
    if (!Array.isArray(act.memberIds) || !act.memberIds.includes(creatorId)) return;
...
function registerAct(act, { reason = "Created" } = {}) {
  if (!act) return null;
  if (!Array.isArray(state.acts)) state.acts = [];
  if (!act.status) act.status = "Active";
  if (!act.statusReason) act.statusReason = null;
  if (!Number.isFinite(act.statusAt)) act.statusAt = state.time?.epochMs ?? Date.now();
  state.acts.push(act);
  const actKind = resolveActKindFromType(act.type);
...
function refreshDailyMarket() {
  ...
  if (legacy.length) {
    ...
    legacy.forEach((creator) => {
      if (existingIds.has(creator.id)) return;
      const next = normalizeCreator(creator);
      markCreatorUnsigned(next);
      next.signCost = computeSignCost(next);
...
function startEraForAct({ actId, name, rolloutId, contentType, contentId, source }) {
  ...
  if (act) setActActive(act, "Era started");
...
function resolveDistributionLeadWeeks(distribution, { rush = false } = {}) {
  ...
  if ((dist === "Physical" || dist === "Both")) return config.physicalLeadWeeks;
  return rush ? 0 : config.digitalLeadWeeks;
}

function getReleaseRushFee(distribution, { rush = false } = {}) {
  if (!rush) return 0;
  if (distribution !== "Digital") return 0;
  const config = resolveReleaseDistributionConfig();
...
function resolveReleaseDistributionForTrack(track, distribution, { labelFans, log } = {}) {
  ...
  if (dist !== "Physical" && dist !== "Both") return { distribution: dist, downgraded: false, eligibility: null };
  const eligibility = evaluatePhysicalEligibility(track, { labelFans });
  if (eligibility.ok) return { distribution: dist, downgraded: false, eligibility };
  if (log) {
    const reasons = [];
    if (!eligibility.qualityOk) reasons.push(`quality ${eligibility.quality ?? "?"} < ${eligibility.minQuality}`);
    if (!eligibility.fansOk) reasons.push(`fans ${formatCount(eligibility.fans ?? 0)} < ${formatCount(eligibility.minFans)}`);
...
function applyReleaseFees({ distribution, rush = false, track = null } = {}) {
  ...
  if (total) state.label.cash -= total;
  if (track && distFee) recordTrackDistributionFee(track, distFee);
  if (track && rushFee) recordTrackDistributionFee(track, rushFee);
...
function roundPhysicalUnits(value) {
  const roundTo = Number.isFinite(ECONOMY_BASELINES?.physicalRunRound) ? ECONOMY_BASELINES.physicalRunRound : 50;
  if (!Number.isFinite(roundTo) || roundTo <= 0) return Math.max(0, Math.round(value));
...
function ensureTrackPhysicalInventory(track) {
  ...
  if (!Number.isFinite(inventory.createdAt)) inventory.createdAt = null;
  if (!Number.isFinite(inventory.lastWeek)) inventory.lastWeek = null;
  if (!Number.isFinite(inventory.lastSaleWeek)) inventory.lastSaleWeek = null;
  if (typeof inventory.source !== "string") inventory.source = inventory.source || null;
  return inventory;
}

function initShelvedPhysicalInventory(track, { act = null, label = state.label } = {}) {
  if (!track || (track.distribution !== "Physical" && track.distribution !== "Both")) return null;
...
function applyShelvedPhysicalSales() {
  ...
  const week = weekIndex() + 1;
  state.tracks.forEach((track) => {
    if (!isTrackReleaseShelved(track)) return;
    const act = track.actId ? getAct(track.actId) : null;
    const inventory = initShelvedPhysicalInventory(track, { act, label: state.label });
    if (!inventory || inventory.unitsAvailable <= 0) return;
...
function scheduleRelease(track, hoursFromNow, distribution, note, { rush = false } = {}) {
  ...
  if (feeResult.rushFee) feeParts.push(`Rush fee: ${formatMoney(feeResult.rushFee)}`);
...
function bankPreReleaseMomentum(track, boostWeeks, options = {}) {
  if (!track) return { ok: false, banked: 0 };
  ...
  if (shouldLog) {
    ...
    logEvent(
      `Banked pre-release momentum${source}: "${track.title}" +${safeBoost}w -> ${next}w (${releaseLabel}${releaseNote}).`
    );
  }
...
function archiveMarketTracks(entries) {
  ...
  const archived = entries
    ...
    .map((entry) => {
      ...
      const hasChartHistory = Object.values(chartHistory).some((history) => (
        Number.isFinite(history?.peak) || Number.isFinite(history?.weeks)
      ));
      const base = {
...
function updateEconomy(globalScores) {
  ...
  if (physicalCount) {
    reportLines.push(
      `Shelved physical ${formatMoney(physicalRevenue)} | Costs ${formatMoney(physicalCosts)} | Net ${formatMoney(physicalProfit)}`
    );
  }
  postSocial({
    handle: "@eyeriStats",
...
function resolveInternalTrackCharts() {
  ...
  if (!Array.isArray(tracks.global)) return null;
  return tracks;
}

function resolveInternalPromoCharts() {
  const promos = state.meta?.chartEvaluation?.promos;
  if (!promos || typeof promos !== "object") return null;
  if (!Array.isArray(promos.global)) return null;
  return promos;
}

function resolveInternalTourCharts() {
  const tours = state.meta?.chartEvaluation?.tours;
  if (!tours || typeof tours !== "object") return null;
  if (!Array.isArray(tours.global)) return null;
  return tours;
}
...
function selectTopKEntries(entries, k) {
  ...
  if (!limit) return [];
  if (limit >= entries.length) return entries.slice().sort(compareScoreEntries);
  const heap = [];
...
function ensureChartEvaluationStore() {
  ...
  if (!Number.isFinite(store.weekIndex)) store.weekIndex = null;
  ...
  if (!Array.isArray(store.tracks.global)) store.tracks.global = [];
  if (!store.tracks.nations || typeof store.tracks.nations !== "object") store.tracks.nations = {};
  if (!store.tracks.regions || typeof store.tracks.regions !== "object") store.tracks.regions = {};
  ...
  if (!store.dominance.nations || typeof store.dominance.nations !== "object") store.dominance.nations = {};
  if (!store.dominance.regions || typeof store.dominance.regions !== "object") store.dominance.regions = {};
  return store;
}

function applyTrackChartEvaluation(internalCharts, evalSizes) {
  if (!internalCharts || typeof internalCharts !== "object") return null;
...
function findChartMonopoly() {
  ...
  for (let i = 0; i < checks.length; i += 1) {
    ...
    if (label) {
      ...
      logEvent(
        `Monopoly detected: scope=${check.scope} type=${check.type} k=${check.size} labelId=${label} week=${week} ts=${stamp}.`,
        "warn"
      );
...
function checkWinLoss(scores) {
  ...
  if (state.meta.monopolyLoss) return;
...
function ensureRivalAutoOpsState(rival) {
  ...
  if (typeof rival.autoOps.eraIndex !== "number") rival.autoOps.eraIndex = 0;
  if (typeof rival.autoOps.lastPlannedWeek !== "number") rival.autoOps.lastPlannedWeek = null;
  return rival.autoOps;
}

function ensureRivalAutoOpsAct(rival) {
  const autoOps = ensureRivalAutoOpsState(rival);
  if (!autoOps) return null;
  if (autoOps.act && autoOps.act.id && autoOps.act.name) return autoOps.act;
...
function updateRivalAutoOpsEraProgress(era, currentWeekNumber) {
  if (!era || !Number.isFinite(era.startedWeek)) return era;
...
function estimateRivalTourReserve(husk, walletCash) {
  ...
  const baseCost = Math.round(
    Math.max(0, Number(tier?.venueFee || 0) + Number(tier?.staffingBase || 0) + Number(tier?.travelBase || 0))
  );
...
function spendRivalTourBudget(budget, cost) {
  if (!budget) return { ok: false, reason: "tour budget missing." };
  const safeCost = Number.isFinite(cost) ? Math.max(0, Math.round(cost)) : 0;
  if (!safeCost) return { ok: false, reason: "tour cost unavailable." };
...
function scheduleRivalTourStep({
  rival,
  act,
  era,
  husk,
  ...
  if (!rival || !act || !budget) return { ok: false, kind: "budget", reason: "missing rival tour context." };
  const anchor = pickRivalTourAnchorRelease(rival, targetWeekIndex + 1);
  if (!anchor) return { ok: false, kind: "capacity", reason: "no tour anchor release." };
  ...
  if (scheduledAt <= now) return { ok: false, kind: "venue", reason: "tour window already passed." };
  const venues = listTourVenues({
    nation: rival.country || null,
    regionId: null,
    tier: null
  });
  if (!venues.length) return { ok: false, kind: "venue", reason: "no venues available." };
  const dayKey = tourDayKey(scheduledAt);
  ...
  if (dayConflict) return { ok: false, kind: "stamina", reason: "tour date conflicts with another booking." };
  ...
  if (!scored.length) return { ok: false, kind: "venue", reason: "venue availability blocked." };
  ...
  if (!spend.ok) return { ok: false, kind: "budget", reason: spend.reason };
...
function resolveInternalChartEntries(scopeType, scopeKey) {
  ...
  if (scopeType === "global") return tracks.global || [];
  if (scopeType === "nation") return tracks.nations?.[scopeKey] || [];
  if (scopeType === "region") return tracks.regions?.[scopeKey] || [];
  return null;
}
...
function selectRivalDominanceTarget(rival) {
  ...
  if (focus === "REQ-03") {
    ...
    if (region) return { scopeType: "region", scopeKey: region.id, size: evalChartSizes.region };
  }
  const region = pickRivalDominanceRegion(rival);
  if (region) return { scopeType: "region", scopeKey: region.id, size: evalChartSizes.region };
  const nation = NATIONS.includes(rival.country) ? rival.country : NATIONS[0];
...
function runRivalAutoOpsWeek(stateRef = state, weekContext = {}) {
  if (!Array.isArray(stateRef.rivals) || !stateRef.rivals.length) return;
  const huskLibrary = buildHuskLibrary();
  ...
  stateRef.rivals.forEach((rival) => {
    ...
    const autoOps = ensureRivalAutoOpsState(rival);
    if (autoOps?.lastPlannedWeek === currentWeek) return;
    ...
    if (planBudget.walletCash < planBudget.reserveFloor) {
      logRivalAutoOpsBlocker(
        rival,
        "reserve",
        `reserve floor ${formatMoney(planBudget.reserveFloor)} exceeds cash ${formatMoney(planBudget.walletCash)}.`,
        currentWeek
      );
    }
  ...
  if (market) {
    ...
    if (next > current && boostWeeks > 0) {
      const releaseLabel = Number.isFinite(scheduledRelease.releaseAt)
        ...
        : "TBD";
      logEvent(
        `${rival.name} banked pre-release momentum: "${scheduledRelease.title || "Scheduled release"}" +${boostWeeks}w -> ${next}w (Release ${releaseLabel}).`
      );
    }
  }
  recordPromoContent({
...
async function tick(now) {
  ...
  if (samplePerf) markPerf("rls:tick:start");
  ...
  if (secPerQuarter !== Infinity) {
    ...
    if (queuedIterations > 0) {
      ...
      if (samplePerf) markPerf("rls:tick:sim:start");
  ...
  if (samplePerf) markPerf("rls:tick:sync:start");
...
function saveToSlot(index) {
  ...
  if (localStorageAvailable && !session.localStorageDisabled) {
    ...
    if (projectedBytes > LOCAL_SAVE_QUOTA_BYTES) {
      warnLocalStorageIssue(
        `Local save is large (${formatByteSize(payloadBytes)}). Attempting write; consider External Storage for larger saves.`
      );
    }
    try {
...
function normalizeState() {
  ...
  state.acts = state.acts.filter(Boolean).map((act) => {
    ...
    if (typeof act.status !== "string" || !["Active", "Legacy"].includes(act.status)) act.status = "Active";
    if (typeof act.statusReason !== "string") act.statusReason = act.statusReason || null;
    if (typeof act.statusAt !== "number") act.statusAt = state.time?.epochMs ?? Date.now();
  ...
  touring.bookings = touring.bookings.filter(Boolean).map((booking) => {
    ...
    if (next.catalogMerch && typeof next.catalogMerch === "object") {
      ...
      if (!Number.isFinite(merch.unitPrice)) merch.unitPrice = Math.round(Number(merch.unitPrice || 0) * 100) / 100;
      if (!Number.isFinite(merch.unitCost)) merch.unitCost = Math.round(Number(merch.unitCost || 0) * 100) / 100;
  ...
  if (state.meta?.marketTrackArchive?.length) {
    state.meta.marketTrackArchive = state.meta.marketTrackArchive.filter(Boolean).map((entry) => {
      ...
      if (!Number.isFinite(entry.trendRankAtRelease)) entry.trendRankAtRelease = null;
      if (!Number.isFinite(entry.trendTotalAtRelease)) entry.trendTotalAtRelease = null;
      if (typeof entry.huskId !== "string") entry.huskId = entry.huskId || null;
      if (typeof entry.huskSource !== "string") entry.huskSource = entry.huskSource || null;
      if (typeof entry.rolloutPlanId !== "string") entry.rolloutPlanId = entry.rolloutPlanId || null;
      if (typeof entry.rolloutPlanSource !== "string") entry.rolloutPlanSource = entry.rolloutPlanSource || null;
      if (typeof entry.rolloutFocusType !== "string") entry.rolloutFocusType = entry.rolloutFocusType || null;
      if (typeof entry.rolloutFocusId !== "string") entry.rolloutFocusId = entry.rolloutFocusId || null;
